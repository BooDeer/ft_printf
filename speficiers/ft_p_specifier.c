...
/*   Updated: 2020/01/01 18:30:54 by hboudhir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_p_specifier(va_list args, t_list *node)
{
	...
	if (!p->width)
		p->width = ft_strdup("1");
	(p->precision || node->flag[i - 1] == '.') ? (p->precision = ft_strdup("1")) :  (p->precision = ft_strdup("0"));
	...
	{	p->precision = ft_strdup("");
		...
		p->precision = ft_strjoin("0x", p->precision);	
	if (ft_atoi(p->width) > (int)ft_strlen(p->precision) && ft_atoi(p->width) > len)
	{
		...
		if (p->flag == '-')
		{
			...
			if (len > (int)ft_strlen(p->precision))
				ft_memcpy(&p->string[len - ft_strlen(p->precision)], p->precision, ft_strlen(p->precision));
			else
				ft_memcpy(p->string, p->precision, ft_strlen(p->precision));
		}
		else if (p->flag == '0')
		{
		...
		else
		{
			...
			ft_memcpy(&p->string[ft_atoi(p->width) - ft_strlen(p->precision)], p->precision, ft_strlen(p->precision));
		}
	...
	else if (len > (int)ft_strlen(p->precision))
	{
		...
		ft_memset(p->string, '0', len);
		ft_strlcpy(&p->string[len - ft_strlen(p->precision)], p->precision, ft_strlen(p->precision) + 1);
		p->string[len] = '\0';
	}
	else
	{
